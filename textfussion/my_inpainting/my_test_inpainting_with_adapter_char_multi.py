...
output_data_dir = "./demo/inpainting_with_adapter_char_text_guide_with_loss_mask_epoch_2e_lr_1e-5_10"

checkpoint_dir = "./output/0328_inpainting_with_single_adapter_char_text_guide_with_loss_mask_epoch_2e_lr_1e-5"
...
def image_inpainting(pipe, img_index, prompt_text, adapter=None):
    ...
    image = pipe(prompt=prompt, image=init_image, mask_image=mask_image, adapter=adapter).images[0]
...
if __name__ == '__main__':
    ...
    pipe = StableDiffusionInpaintWithAdapterPipeline(vae, text_encoder, tokenizer, unet, noise_scheduler, None)
    pipe = pipe.to("cuda")
