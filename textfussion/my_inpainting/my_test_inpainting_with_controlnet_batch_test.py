...
from src.pipelines.stable_diffusion_inpainting_mask_controlnet \
    import StableDiffusionInpaintMaskControlnetPipeline
...
output_data_dir = "./demo/0426_controlnet_mask_pose_no_adjust"

checkpoint_dir = "./output/0424_controlnet_mask_pose"
...
if __name__ == '__main__':
    ...
    pipe = StableDiffusionInpaintMaskControlnetPipeline(vae, text_encoder, tokenizer, unet, noise_scheduler, None)
    pipe = pipe.to("cuda")
    adapter = ControlNet()
    adapter.load_state_dict(torch.load(os.path.join(checkpoint_dir, "control_net.pth")))
