...
class OursDataset(Dataset):
    ...
    def __init__(
        ...
        self.data_dir = "/home/lfu/project/preprocessing_scene_text/text_diffusion_data_v5"
        self.size = size
    ...
    def _load_images_paths(self):
        ...
        load_data_list = []
        with open(os.path.join(self.data_dir, "metadata.jsonl"), "r", encoding='utf-8') as f:
            for line_item in jsonlines.Reader(f):
                load_data_list.append(line_item)

        # load_data_list = load_data_list[::300]
    ...
    def __getitem__(self, index):
        ...
        instance_image = cv2.imread(os.path.join(self.data_dir, data_item["file_name"]))
        ...
        # ipdb.set_trace()
        instance_image = cv2.resize(instance_image, (512, 512))
        instance_image = np.array(instance_image).transpose(2, 0, 1)
        instance_image = torch.from_numpy(instance_image).to(dtype=torch.float32) / 127.5 - 1.0
...
def main():
    ...
    # ipdb.set_trace()
    # print(vae.encoder.conv_in.weight.data)
    # print(vae.quant_conv.weight.data)
    # print(vae.post_quant_conv.weight.data)
    # vae.load_state_dict(torch.load("output/text_vae_pretrain/text_vae.pth"))
    # print(vae.encoder.conv_in.weight.data)
    # print(vae.quant_conv.weight.data)
    # print(vae.post_quant_conv.weight.data)
    ...
    torch.save(vae.module.state_dict(), os.path.join(args.output_dir, "text_vae.pth"))
