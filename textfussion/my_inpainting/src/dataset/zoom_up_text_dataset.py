...
class ZoomUpTextDataset(Dataset):

    def __init__(self, data_dir, resolution=512):

        self.zoom_up_resolution = 256
    ...
    def __getitem__(self, idx):
        ...
        # skeleton = self.get_position_guide(data_item["text"], mask)
        ...
        # text = "fill the region with the text '" + data_item["text"] + "', and keep the background unchanged."

        # ipdb.set_trace()
        text = "Write english text '" + data_item["text"] + "'."

        sample = {'pixel_values': image, 'input_ids': text, 'mask': mask,
                  'skeleton': skeleton, 'ori_text': data_item["text"]}
    ...
    def get_mask_position_guide(self, text, mask_image, ori_image):
        ...
        if len(contours) < 1:
            ...
            ori_image = ori_image.astype(np.float32) / 255.0 - 1.0
