...
class WithPrePromptAdapter(nn.Module):
    def __init__(self, channels=[320, 640, 1280, 1280], nums_rb=3, cin=64, ksize=3, sk=True, use_conv=True):
        ...
        # encoder_layer = nn.TransformerEncoderLayer(char_embed_dim * 2, nhead=8)
        # self.transformer_encoder = nn.TransformerEncoder(encoder_layer, num_layers=2)

        self.init_layers = [self.body]
        self.initialize_weights()

        # self.zero_init_layers = [self.transformer_encoder]
