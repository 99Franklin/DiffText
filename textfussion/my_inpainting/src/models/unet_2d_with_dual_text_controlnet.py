...
from .unet_2d_blocks_with_dual_text_controlnet import get_down_block, get_up_block, UNetMidBlock2DCrossAttn
...
class UNet2DConditionWithDualTextControlNetModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    r"""
    ...
    def forward(
        ...
        if extra_condition is not None:
            sample = sample + extra_condition.pop()
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            extra_condition_samples = extra_condition[-len(upsample_block.resnets):]
            extra_condition = extra_condition[: -len(upsample_block.resnets)]
            ...
            if hasattr(upsample_block, "has_cross_attention") and upsample_block.has_cross_attention:
                sample = upsample_block(
                    ...
                    attention_mask=attention_mask,
                    extra_condition_tuple=extra_condition_samples,
                    char_embedding=char_embedding,
                )
            else:
                sample = upsample_block(
                    hidden_states=sample, temb=emb, res_hidden_states_tuple=res_samples, upsample_size=upsample_size,
                    extra_condition_tuple=extra_condition_samples, char_embedding=char_embedding,
                )
